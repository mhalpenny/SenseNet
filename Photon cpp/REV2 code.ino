  ...
  gyro.getEvent(&event);
    ...
    //    ipAddress = udp.remoteIP();
      // port = udp.remotePort();
        ...
        if( inMessage.parse())
        {
            inMessage.route("/rev1", PING);
            inMessage.route("/rev1B", PING2);
        }
    }
...
if(!eventCompleteB){
    ...
    if(millis() > (eventStartB + (unsigned long) 7000)){
        eventCompleteB = true;
        Serial.println("end");
    }
...
void  p2p(int val) {
  IPAddress ipAddress(192,168,1,103);
  unsigned int localPort = 8888;
